...
for j = 1:quto
    ...
    while i > 0 
        ...
        index = i - gamma(i) + x;
        if res < stor_data{1,gamma(i)+1,index} % (q-1)^gamma(i)*N_q(1,index)
            a(i) = 0;
        elseif res >= stor_data{q-1,gamma(i)+1,index} % (q-1)^(gamma(i)+1)*N_q(1,index)
            a(i) = q-1;
            res = res - stor_data{q-1,gamma(i)+1,index}; % (q-1)^(gamma(i)+1)*N_q(1,index);
        else
            for l = 1:q-2
