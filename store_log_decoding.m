...
function bin_mess = store_log_decoding(CQA_LOCO,stor_data,m,x,q,s_c) 
...
%     while i > 0 
    for i = 1:m
        ...
        if a(i) ~= 0
            res = a(i);
            for k = r:-1:1
                if res >= stor_data{k,1,1+x}
                    rank = rank + stor_data{k,gamma(i)+1,index};
                    res = res - stor_data{k,1,1+x};
                end
            end
        end
    end
